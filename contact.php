  ...
  <section class="section-two d-flex flex-wrap justify-content-center text-align-center">
    ...
    <div id="contact" class="right col-12 col-lg-6 col-md-6">
      ...
      <form action="proc-contact?token=<?= $_GET['token'];?>" method="post">
        ...
        <input type="text" readonly value="<?= get_val('product', 'token', $_GET['token'], 'destination') ;?> " name="destination"><br><br>
        <label for="#">Price:</label><br>
        <input type="text" readonly value="<?= number_format(get_val('product', 'token', $_GET['token'], 'price')) ;?> "  name="price"><br><br>
        <?php } ?>
